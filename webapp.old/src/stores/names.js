...
async function listen() {
  ...
  sub.subscribe({
    next: result => _set({ status: "loaded", data: result.data.namedEntities.map(item => item) }),
...
export default dataStore = {
  ...
  load: async () => {
    ...
    _set({ status: "loaded", data: result.data.namedEntities.map(item => item) });
    return { data: result.data };
  },
  boot: data => {
    if (data) {
      client.writeQuery({ query: NAMES, data });
      _set({ status: "loaded", data: data.namedEntities.map(item => item) });
    }
    listen();
  },
  listen
